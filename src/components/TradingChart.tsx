...
// Build candles incrementally — closed candles are locked, only the forming
// candle at the end is recalculated from live ticks.
function buildCandles(
  data: number[],
  ticksPerCandle: number,
  lockedRef: React.MutableRefObject<Candle[]>,
  lockedTickCountRef: React.MutableRefObject<number>
): Candle[] {
  const totalTicks = data.length;
  const currentLocked = lockedRef.current;
  const currentLockedTicks = lockedTickCountRef.current;

  // How many complete candles does the current data support?
  const completeCandles = Math.floor(totalTicks / ticksPerCandle);

  // If we have more complete candles than locked, seal the new ones.
  if (completeCandles > currentLocked.length) {
    const newLocked: Candle[] = [...currentLocked];
    for (let i = currentLocked.length; i < completeCandles; i++) {
      const start = i * ticksPerCandle;
      const group = data.slice(start, start + ticksPerCandle);
      newLocked.push({
        o: group[0],
    ...
    lockedTickCountRef.current = completeCandles * ticksPerCandle;
  }

  // Build the forming candle from remaining ticks (may be empty).
  const formingStart = lockedRef.current.length * ticksPerCandle;
  const formingTicks = data.slice(formingStart);

  if (!formingTicks.length) return lockedRef.current;

  const forming: Candle = {
    o: formingTicks[0],
    c: formingTicks[formingTicks.length - 1],
    h: Math.max(...formingTicks),
    l: Math.min(...formingTicks),
  };

  return [...lockedRef.current, forming];
}
...
export default function TradingChart({
  ...
  // Locked (immutable) candles — sealed when their tick window completes.
  const lockedCandlesRef = useRef<Candle[]>([]);
  const lockedTickCountRef = useRef<number>(0);
  ...
  // Reset locked candles when timeframe changes — different tick windows.
  useEffect(() => {
    lockedCandlesRef.current = [];
    lockedTickCountRef.current = 0;
    setOffset(0);
  ...
  const allCandles =
    type === "candle"
      ? buildCandles(data, ticksPerCandle, lockedCandlesRef, lockedTickCountRef)
      : [];
  ...
  const clampedVisible = Math.min(
    Math.max(MIN_VISIBLE, visibleCount),
    Math.min(MAX_VISIBLE, Math.max(1, allCandles.length))
  );
  ...
  const clientToChartData = useCallback(
    (clientX: number, clientY: number) => {
      ...
      if (type === "candle" && visibleCandles.length) {
        ...
        const idx = Math.round(
          ((svgX - padX) / (chartW - padX * 2)) * (visibleLine.length - 1)
        );
    ...
    (e: React.MouseEvent) => {
      if (dragRef.current) {
        ...
        const newOffset = Math.min(
          Math.max(0, dragRef.current.startOffset + delta),
          maxOffset
        );
        setOffset(newOffset);
      } else {
  ...
  const onWheel = useCallback((e: React.WheelEvent) => {
    ...
    if (e.shiftKey) {
      ...
      setVisibleCount((v) =>
        Math.min(Math.max(MIN_VISIBLE, Math.round(v * factor)), MAX_VISIBLE)
      );
    }
  }, []);
  ...
  const crosshairEl = crosshair ? (
    <g>
      <line
        x1={crosshair.x} x2={crosshair.x} y1={padY} y2={height - padY}
        stroke="rgba(255,255,255,0.35)" strokeWidth={1} strokeDasharray="3 3"
      />
      <line
        x1={padX} x2={chartW - padX} y1={crosshair.y} y2={crosshair.y}
        stroke="rgba(255,255,255,0.35)" strokeWidth={1} strokeDasharray="3 3"
      />
  ...
  const tooltipEl =
    crosshair?.candle ?
      (() => {
        ...
        const lines = [
          { label: "Open",  value: c.o.toFixed(4), col },
          { label: "High",  value: c.h.toFixed(4), col: "rgba(255,255,255,0.9)" },
          { label: "Low",   value: c.l.toFixed(4), col: "rgba(255,255,255,0.9)" },
          { label: "Close", value: c.c.toFixed(4), col },
        ];
        const boxW = 160;
        const boxH = 104;
        const bx =
          crosshair.x + 14 > chartW - boxW ? crosshair.x - boxW - 8 : crosshair.x + 14;
            ...
            {lines.map((line, i) => (
              <g key={i}>
                ...
                <text x={bx + boxW - 12} y={by + 22 + i * 22}
                  fill={line.col} fontSize={13} fontFamily="monospace"
  ...
  const currentLabel =
    currentPrice > 0 ? (
      <g>
        ...
        <rect x={chartW} y={currentY - 9} width={labelW - 2} height={18}
  ...
  const entryLine = entryPrice ? (
    <g>
      <line x1={padX} x2={chartW - padX}
        y1={scaleY(entryPrice)} y2={scaleY(entryPrice)}
        stroke="#facc15" strokeWidth={1} strokeDasharray="6 3" opacity={0.7} />
      <rect x={chartW} y={scaleY(entryPrice) - 9} width={labelW - 2} height={18}
        fill="#facc15" rx={3} opacity={0.85} />
  ...
  const liqLine = liquidationPrice ? (
    <g>
      <line x1={padX} x2={chartW - padX}
        y1={scaleY(liquidationPrice)} y2={scaleY(liquidationPrice)}
        stroke="#f87171" strokeWidth={1} strokeDasharray="4 3" opacity={0.6} />
      <rect x={chartW} y={scaleY(liquidationPrice) - 9} width={labelW - 2} height={18}
  ...
  const points = visibleLine
    .map((v, i) => `${scaleX(i, visibleLine.length)},${scaleY(v)}`)
    .join(" ");
  const area = [
      ...
      <svg {...svgProps}>
        ...
        {crosshair && (
          <g>
            ...
            <text
              x={
                (crosshair.x + 10 > chartW - 90 ? crosshair.x - 95 : crosshair.x + 10) + 10
              }
